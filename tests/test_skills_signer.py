...
import base64
from pathlib import Path  # used in pytest tmp_path parameter
...
# SignerFeedState lives in session_buddy.mcp (not in the skills_signer package
...
class TestCanonicalPayloadForSigning:
    def test_strips_signature_field_from_dict(self) -> None:
        ...
        assert canonical == canonicalize_payload({"name": "session_buddy-search", "version": "1.0.0"})
    ...
    def test_strips_signature_field_from_pydantic_model(self) -> None:
        ...
        assert b"signature" not in canonical
        assert b"server_pubkey_id" not in canonical
        assert canonical == b'{"name":"session_buddy-search"}'
...
class TestSigningRoundtrip:
    def test_sign_then_verify_succeeds(self) -> None:
        ...
        metadata = {"name": "session_buddy-search", "version": "1.0.0", "tool_refs": []}
        canonical = canonicalize_payload(metadata)
        signed = signer.sign(canonical)
    ...
    def test_tampered_payload_fails_verify(self) -> None:
        ...
        canonical = canonicalize_payload({"name": "session_buddy-search", "version": "1.0.0"})
        signed = signer.sign(canonical)
...
class TestEndToEndFlow:
    def test_server_signs_client_verifies(self) -> None:
        ...
        server_metadata = {
            "name": "session_buddy-search-insights",
            "version": "1.0.0",
            "description": "Search across Akosha-indexed systems",
            "tool_refs": ["mcp__session_buddy__search_all_systems"],
            "dependencies": [],
        }
