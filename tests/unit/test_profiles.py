...
class TestMandatoryRegistrations:
    ...
    def test_mandatory_groups_count(self):
        """MANDATORY_GROUPS contains health, baseline, and skills_signer registrations.

        Phase 1.5 (plan §10.3.1): skills_signer is infrastructure-critical
        and must be present at every profile tier (signing verification
        is on every Phase 2/6 install).
        """
        assert len(SESSION_BUDDY_MANDATORY_GROUPS) == 3
