...
@pytest.mark.unit
class TestInitDefaultStorage:
    """Tests for CICDTracker default-storage initialization path."""

    def test_init_without_storage_creates_real_storage(
        self, tmp_path: Path
    ) -> None:
        """Test the ``else`` branch where storage is auto-created from db_path.

        Coverage gap: lines 207-209. When the caller omits the ``storage``
        kwarg, the constructor must import SkillsStorage lazily and
        instantiate it with the provided db_path.
        """
        from session_buddy.integrations.cicd_tracker import CICDTracker

        db_path = tmp_path / "skills.db"
        tracker = CICDTracker(db_path=db_path)

        # Real SkillsStorage should be created with the right db_path.
        assert tracker.db_path == db_path
        assert tracker.storage is not None
        assert Path(tracker.storage.db_path) == db_path

    def test_init_default_storage_initialize_metrics(self, tmp_path: Path) -> None:
        """Test that the default-storage branch still seeds stage_metrics."""
        from session_buddy.integrations.cicd_tracker import CICDTracker

        tracker = CICDTracker(db_path=tmp_path / "skills.db")
...
@pytest.mark.unit
class TestTrackFailedNoErrorMessage:
    """Coverage gap: branch 284->exit when failed but error_message is None."""

    def test_failed_invocation_without_error_message_does_not_record_failure(
        self,
    ) -> None:
        """Test that a failed invocation with ``error_message=None`` skips the common_failures update.

        When ``completed=False`` but ``error_message`` is ``None``, the
        branch ``if error_message:`` must evaluate to False, and the
        common_failures counter must NOT be incremented.
        """
        from session_buddy.integrations.cicd_tracker import CICDTracker
        ...
        assert m.failed_runs == 1
        assert m.successful_runs == 0
        # common_failures must remain empty because error_message is None.
        assert m.common_failures == {}


@pytest.mark.unit
class TestRecommendationsNoMissingSkills:
    """Coverage gap: branch 522->514 when missing skills set is empty."""

    def test_no_missing_skills_recommendation_when_all_used(
        self,
    ) -> None:
        """Test that no missing-skills recommendation is added when all expected skills are used."""
        from session_buddy.integrations.cicd_tracker import CICDTracker
        from unittest.mock import MagicMock
        ...
        # Track ALL expected skills for the "test" stage so the
        # ``expected_skills - used_skills`` set is empty.
        for skill in CICDTracker.STAGE_SKILLS["test"]:
            tracker.track_pipeline_stage(
                context=_make_context(),
                stage_name="test",
                skill_name=skill,
        ...
        # All expected skills used -> no "missing skills" recommendation.
        assert missing_recs == []


@pytest.mark.unit
class TestReportEmptySkillsUsed:
    """Coverage gap: branch 628->633 when skills_used is empty in a stage."""

    def test_report_handles_stage_with_empty_skills_used(
        self,
    ) -> None:
        """Test the report path where skills_used is an empty set.

        We simulate this by populating the stage's metrics with a run
        (so it shows up in stage_analytics) but then clearing the
        skills_used set — emulating the edge case where a stage appears
        in analytics but never recorded any skill name.
        """
        from session_buddy.integrations.cicd_tracker import CICDTracker
        from unittest.mock import MagicMock
        ...
        # Record a successful run to add a stage to stage_analytics.
        tracker.track_pipeline_stage(
            context=_make_context(),
            stage_name="test",
            skill_name="pytest-run",
        ...
        # Force the skills_used set to be empty so the
        # ``if stage_data["skills_used"]:`` branch is False.
        tracker.stage_metrics["test"].skills_used.clear()
        # Add a common failure so the "common_failures" branch is
        # exercised on the same iteration of the report loop.
        tracker.stage_metrics["test"].common_failures["err"] = 1

        report = tracker.generate_pipeline_report("p", days=7)

        # Skills Used section is skipped; Common Failures still appears.
        assert "Common Failures:" in report
