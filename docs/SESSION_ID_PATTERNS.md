...
1. **Time Ordering:** Natural sortability by ULID = chronological ordering
1. **Distributed Safe:** No coordination needed for session ID generation
1. **Embedded Timestamp:** Extract timestamp from ULID without extra column
1. **Vector Compatibility:** ULID can coexist with existing embedding columns
1. **Semantic Search:** ULID + embedding enables powerful time-based semantic queries

## Next Steps

1. ✅ **COMPLETED:** Analysis of current Session-Buddy identifier patterns
1. **NEXT:** Replace session ID generation with ULID from Oneiric
1. **NEXT:** Update reflection/conversation ID generation to ULID
1. **NEXT:** Add ULID validation to MCP tools
