...
**Lesson**: Python 3.12+ PEP 695 type parameters (`def func[T](...)`) make the type parameter name available in scope, eliminating the need for string literals.
...
2. ❌ Swapped decorator order: `@model_validator(mode="before")` before `@classmethod`
3. ✅ **Removed @classmethod decorator entirely**
...
2. session_buddy/di/__init__.py
3. session_buddy/settings.py
4. session_buddy/server_optimized.py
5. session_buddy/server.py
...
- [CLAUDE.md - Development Guidelines](../CLAUDE.md#coding-standards--best-practices)
- [CLAUDE.md - Type Safety Requirements](../CLAUDE.md#type-safety-requirements)
- [Crackerjack Integration](../CLAUDE.md#crackerjack-integration-for-session-buddy)
...
2. Pydantic v2 model validator decorators
3. Explicit type casting for dependency injection
4. Proper logger method signatures
5. Precise type annotations without `Any`
