...
async def test_multi_point_capture_with_deduplication():
...
This is especially important in larger codebases.
        ...
        checkpoint_result = await manager.checkpoint_session(working_directory=str(tmpdir))
        ...
        # Note: The extraction uses SessionMgmtSettings().database_path which defaults
        # to ~/.claude/data/reflection.duckdb, not the test db_path
        async with ReflectionDatabase(
            collection_name="default",
            settings=ReflectionAdapterSettings(
                database_path=Path.home() / ".claude" / "data" / "reflection.duckdb",
                collection_name="default",
            ),
        ) as db:
        ...
        session_end_result = await manager.end_session(working_directory=str(tmpdir))
...
This is critical for web applications.
        ...
        session_end_result2 = await manager2.end_session(working_directory=str(tmpdir))
