...
def _get_hooks_manager():
    ...
    Note:
        Uses the DI container for singleton resolution.
    """
    from session_buddy.core.hooks import CodeFormatter, HooksManager

    with suppress(Exception):
        manager = get_sync_typed(HooksManager)  # type: ignore[no-any-return]
        if isinstance(manager, HooksManager):
            return manager

    # Create and register if not found
    # Try to get CodeFormatter from DI, but don't fail if not available
    try:
    ...
    depends.set(HooksManager, manager)
    return manager


def register_hooks_tools(server: FastMCP) -> None:
    ...
    async def list_hooks(
        ...
        hooks_dict = hooks_manager.list_hooks(hook_type_enum)
