...
def _check_dangerous_chars(token_str: str) -> None:
    """Check for dangerous shell metacharacters in token.

    Args:
        token_str: Token to check

    Raises:
        ValueError: If dangerous characters detected
    """
    dangerous_chars = {";", "|", "&", "$", "`", "(", ")", "<", ">", "\n", "\r"}
    if any(char in token_str for char in dangerous_chars):
        raise ValueError(
            f"Blocked potentially unsafe argument: {token_str}. "
            "Dangerous character detected."
        )


def _is_allowed_argument(token_str: str, allowed_args: set[str]) -> bool:
    """Check if argument is in allowlist (handles key=value format).

    Args:
        token_str: Token to validate
        allowed_args: Set of allowed argument names

    Returns:
        True if argument is allowed
    """
    if "=" in token_str:
...
def _is_flag_with_value(token_str: str) -> bool:
    """Check if flag accepts a value.

    Args:
        token_str: Flag token to check

    Returns:
        True if flag accepts a value
    """
    FLAGS_WITH_VALUES = {"--severity", "--confidence", "--output", "--platform"}
    return token_str in FLAGS_WITH_VALUES


def _get_allowed_args() -> set[str]:
    """Get set of allowed Crackerjack arguments.

    Returns:
        Set of allowed argument names
    """
    return {
...
def _parse_crackerjack_args(args: str) -> list[str]:
    """Parse and validate Crackerjack arguments using shlex.

    SECURITY: This function prevents command injection through:
    1. Using shlex.split() for proper quote handling (respects shell escaping)
    2. Validating against an allowlist of permitted arguments
    3. Blocking shell metacharacters that could enable injection
    4. Validating argument formats (flags, key=value pairs)

    This replaces the emergency fix with production-quality parsing that
    handles quoted arguments correctly while maintaining security.

    Args:
        args: Arguments string to parse and validate

    Returns:
        List of validated argument tokens

    Raises:
        ValueError: If args contain dangerous patterns or invalid syntax

    Examples:
        >>> _parse_crackerjack_args('--verbose --strict')
        ['--verbose', '--strict']
        >>> _parse_crackerjack_args('--verbose "--with spaces"')
        ['--verbose', '--with spaces']
        >>> _parse_crackerjack_args('')  # Empty args
        []

    Security:
        - Uses shlex.split() for proper shell-like parsing
        - Blocks shell metacharacters (;, |, &, $, backticks, etc.)
        - Validates against allowlist of safe arguments
        - Prevents command injection through malformed input
    """
    if not args or not args.strip():
        return []

    allowed_args = _get_allowed_args()
    ...
    while i < len(tokens):
        ...
        _check_dangerous_chars(token_str)

        if not _is_allowed_argument(token_str, allowed_args):
            raise ValueError(
                f"Blocked argument: {token_str}. "
                f"Safe arguments: {', '.join(sorted(allowed_args))}"
            )

        if _is_flag_with_value(token_str) and i + 1 < len(tokens):
            i += 1
...
def _should_parse_line(line: str) -> bool:
    ...
    return "..." in line and any(
        marker in line for marker in ("✅", "❌", "Passed", "Failed")
    )
...
def _categorize_hook(
    ...
    if any(marker in line for marker in ("❌", "Failed")):
        failed_hooks.append(hook_name)
    elif any(marker in line for marker in ("✅", "Passed")):
...
def _is_results_section_header(line: str) -> bool:
    ...
    return any(
        header in line
        for header in ("Fast Hook Results:", "Comprehensive Hook Results:")
    )
...
def _is_new_section_start(line: str) -> bool:
    ...
    return any(
        indicator in line for indicator in ("⏳ Started:", "Workflow", "Building")
    )
...
def _analyze_quality_trend_results(
    ...
    for result in results:
        ...
        if any(indicator in content for indicator in ("success", "✅")):
            success_trend.append(timestamp)
        elif any(indicator in content for indicator in ("failed", "error", "❌")):
