...
ALL_TOOLS_REGISTRY: dict[str, str] = {
    ...
    "memory_lineage": "Return the provenance chain for a memory (source_type, source_ref, model, extracted_at).",
    "session_learning_report": "Per-session summary: memories created, reinforced, contradicted, or linked.",
    "optimize_search_results": "Optimize and re-rank an existing search-results payload for a token budget.",
    "_optimize_search_results": "Private alias of optimize_search_results; prefers public name in profiles.",
    "peer_context": "Return peer context (representation_text + recent memories); ACL peer_models:read.",
    "update_peer_model": "Trigger a peer-model update (heuristic or LLM-driven); ACL peer_models:write.",
    "causal_chain": "BFS-walk the causal graph from start_id (cycle-safe, max_depth-capped, LLM-free).",
    "distill_skills_now": "Run the distill skills pipeline; quality floor importance_score >= 0.7 (LLM Cost Ceiling 100/wk).",
    "search_distilled_skills": "Search distill skills by problem / approach / because; ranks by importance_score DESC.",
    "distilled_skill_health": "Staleness report for distill skills; flags those not reinforced within threshold_days (default 90).",
