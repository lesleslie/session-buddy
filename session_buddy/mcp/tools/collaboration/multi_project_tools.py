...
async def _with_coordinator(operation_name: str, operation: Any) -> str:
...
async def _add_project_dependency_op(
    ...
    dependency = await coordinator.add_project_dependency(
        ...
        dependency_type=dependency_type,  # ty: ignore[invalid-argument-type] -- validated against VALID_DEPENDENCY_TYPES by the MCP tool wrapper
        description=description,
    )
...
async def _link_sessions_op(
    ...
    link = await coordinator.link_sessions(
        ...
        link_type=link_type,  # ty: ignore[invalid-argument-type] -- validated against VALID_SESSION_LINK_TYPES by the MCP tool wrapper
        context=context,
    )
...
# ---------------------------------------------------------------------------
# MCP tool implementations (module-level so register_*_tools stays small).
# Each tool validates its inputs then delegates to ``_with_coordinator``
# with an ``op`` closure. Moving the bodies out of ``register_*_tools`` keeps
# its McCabe complexity well below the C901 limit (the inline form pushed it
# past 30 because every ``if err: return err`` counted as a branch).
# ---------------------------------------------------------------------------
...
async def create_project_group(
    ...
    async def op(coordinator: MultiProjectCoordinator) -> str:
        return await _create_project_group_op(coordinator, name, projects, description)
...
async def get_project_dependencies(
    ...
    async def op(coordinator: MultiProjectCoordinator) -> str:
        return await _get_project_dependencies_op(coordinator, project, direction)
...
def register_multi_project_tools(mcp: FastMCP) -> None:
    ...
    Each tool body is defined at module scope above; this function
    only wires the tools onto the FastMCP server.
    """
    mcp.tool()(create_project_group)
    mcp.tool()(add_project_dependency)
    mcp.tool()(link_sessions)
    mcp.tool()(list_project_groups)
    mcp.tool()(get_project_dependencies)
    mcp.tool()(find_related_conversations)
