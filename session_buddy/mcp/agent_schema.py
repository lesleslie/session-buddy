...
# Strict allowlist regex per B-4 / plan §5 task #1. Anchored to the
...
class AgentMetadata(BaseModel):
    ...
    - routing hints (``tools``, ``tool_refs``, ``dependencies``)
    - body integrity (``content_hash``)
    - content classification (``title``, ``description``, ``category``,
      ``owner``, ``status``, ``last_reviewed``, ``scope``)
    - body content (``system_prompt``) — Claude Code reads the body,
      not the metadata, at agent invocation time
    - signing payload (``signature``, ``server_pubkey_id``)
    - audit (``timestamp``, ``model``)

    The ``signature`` and ``server_pubkey_id`` fields are populated by
    :mod:`session_buddy.mcp.tools.agents_tools` AFTER signing. The
    ...
    model_config = ConfigDict(
        extra="forbid",
        # NOTE: ``str_strip_whitespace=True`` is intentionally NOT set.
        # ``system_prompt`` carries the FULL markdown body verbatim and
        # ``content_hash`` is the sha256 of those exact bytes (per Phase 3
        # task #2: ``body == system_prompt``). Global whitespace stripping
        # would mutate ``system_prompt`` (e.g. trim its trailing newline)
        # and break the body / hash consistency guarantee that the
        # installer relies on. Short text fields (description, title,
        # category, owner, last_reviewed) either have explicit ``.strip()``
        # validators (``description``) or are taxonomy labels that do not
        # carry meaningful whitespace.
        # ``validate_assignment`` lets us re-validate when the tool code
        # sets ``metadata.signature`` after construction. Pydantic v2 keeps
        # this opt-in because it has a small cost; here the cost is worth
        # the safety.
    ...
    model: str
    # EXACT tool names (Claude Code frontmatter), NOT regex (per R-5).
    tools: list[str] = Field(default_factory=list)
    # Full body — Claude Code reads this verbatim at invocation time.
    # Unlike Skills (which carry separate ``tool_refs`` / ``allowed_tools``),
    # Agents use a single ``system_prompt`` body that can include
    # ``$ARGUMENTS`` substitutions and structured tool instructions.
    system_prompt: str = ""
    ...
    # the ``system_prompt`` bytes (which equals ``body`` returned by
    # ``mcp__<server>__get_agent``); the client (Phase 2 installer
    # variant for agents) asserts this BEFORE write to
    # ``~/.claude/agents/<server>-<name>.md`` (B-1 / plan §11 B-1).
    content_hash: str
