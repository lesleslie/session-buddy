-- Skills Metrics Storage Schema - V1 Initial Schema
-- Migration: V1__initial_schema
-- Description: Initial skills metrics schema with invocations, metrics, and session tracking
--
-- Tables:
...
CREATE TABLE IF NOT EXISTS skill_metrics (
    ...
    avg_duration_seconds REAL,  -- NULL when no completed invocations
...
CREATE TRIGGER IF NOT EXISTS trg_skill_metrics_after_insert
    ...
    BEGIN
        ...
        -- Update completion tracking (when completed)
        UPDATE skill_metrics SET
        ...
        WHERE skill_name = NEW.skill_name AND NEW.completed = 1;

        -- Update abandonment tracking (when not completed)
        UPDATE skill_metrics SET
            abandoned_invocations = abandoned_invocations + 1
        WHERE skill_name = NEW.skill_name AND NEW.completed = 0;

        -- Update workflow paths (when specified)
        UPDATE skill_metrics SET
            workflow_paths = json_set(
                workflow_paths,
                '$.' || NEW.workflow_path,
                COALESCE(json_extract(workflow_paths, '$.' || NEW.workflow_path), 0) + 1
            )
        WHERE skill_name = NEW.skill_name AND NEW.workflow_path IS NOT NULL;

        -- Update common errors (when error occurred)
        UPDATE skill_metrics SET
            common_errors = json_set(
                common_errors,
                '$.' || NEW.error_type,
                COALESCE(json_extract(common_errors, '$.' || NEW.error_type), 0) + 1
            )
        WHERE skill_name = NEW.skill_name AND NEW.error_type IS NOT NULL;

        -- Update follow-up actions (when specified)
        UPDATE skill_metrics SET
            follow_up_actions = (
                CASE
                    WHEN NEW.follow_up_actions IS NOT NULL THEN
                        json_set(
                            follow_up_actions,
                            '$.' || json_extract(NEW.follow_up_actions, '$[0]'),
                            COALESCE(json_extract(follow_up_actions, '$.' || json_extract(NEW.follow_up_actions, '$[0]')), 0) + 1
                        )
                    ELSE follow_up_actions
                END
            )
        WHERE skill_name = NEW.skill_name;
        ...
        WHERE skill_name = NEW.skill_name
            AND completed_invocations > 0;

        -- Handle case where no invocations are completed
        UPDATE skill_metrics SET
...
-- Note: SQLite doesn't support custom functions with procedural code
-- JSON increment is handled directly in the trigger below
