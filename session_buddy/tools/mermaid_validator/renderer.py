...
def _locate_mermaid_core() -> Path | None:
    ...
    1. ``SESSION_BUDDY_MERMAID_CORE`` env var (operator-pinned). Must pass
    ...
    env_override = os.environ.get("SESSION_BUDDY_MERMAID_CORE")
    if env_override:
        path = Path(env_override).resolve()
        if _is_trusted_mermaid_path(path):
            return path
        raise RuntimeError(
            f"SESSION_BUDDY_MERMAID_CORE={env_override} is not under a "
...
def _locate_jsdom() -> Path | None:
    ...
    Looks for the `node_modules/jsdom/` directory under the session-buddy
    ...
    Override via ``SESSION_BUDDY_JSDOM`` env var for CI where the
    session-buddy repo lives at a different path.
    """
    env_override = os.environ.get("SESSION_BUDDY_JSDOM")
    if env_override:
        path = Path(env_override).resolve()
        if path.is_file():
            return path
        raise RuntimeError(
            f"SESSION_BUDDY_JSDOM={env_override} does not exist or is not a file"
        )

    # Walk up from this file to find the session-buddy repo root.
...
def validate_mermaid_blocks(
    ...
    passed. The Node.js runner is
    `session_buddy/tools/mermaid_validator/validate_mermaid.mjs`,
    ...
    if not jsdom:
        raise RuntimeError(
            ...
            "run `npm install` in the session-buddy repo to install the "
...
def print_errors(errors: list[MermaidValidationError]) -> None:
    """Pretty-print the broken blocks for `session-buddy docs check-mermaid`."""
    if not errors:
